...
export default function ForwardDeploymentSection() {
  ...
  return (
    <section id="forward-deployment" className="py-20 md:py-32 bg-white border-t border-slate-200 relative overflow-hidden">
      <div className="absolute top-1/2 left-0 w-96 h-96 bg-[#00B2A9]/5 rounded-full blur-[120px] pointer-events-none mix-blend-multiply" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        {/* FDE Role Card */}
        <div className="bg-[#F8F9FA] border border-slate-200 rounded-3xl p-7 md:p-10 mb-10 relative overflow-hidden shadow-sm">
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#F8485E] to-[#00B2A9]" />
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <h3 className="text-xl md:text-2xl font-bold text-[#0A1622] mb-3">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 bg-[#F8485E] hover:bg-[#E8236E] text-white text-sm font-semibold px-5 py-3 rounded-xl transition-all shadow-lg shadow-[#F8485E]/25"
              >
                Meet Your FDE Team
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {fdeRole.map((role) => (
                <div key={role} className="bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-slate-700 flex items-center gap-2 shadow-sm">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#00B2A9] shrink-0" />
                  {role}
                </div>
              ))}
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            <h3 className="text-base font-bold mb-5 uppercase tracking-wider text-slate-500">Responsibilities</h3>
            <div className="space-y-2">
              {responsibilities.map((resp, i) => (
                <div
                  key={resp.title}
                  className={`border rounded-2xl overflow-hidden transition-all duration-300 cursor-pointer shadow-sm ${
                    openResp === i
                ...
                >
                  <div className="flex items-center justify-between px-5 py-4">
                    <div className="flex items-center gap-3">
                      <resp.Icon className={`w-4 h-4 ${openResp === i ? "text-[#00B2A9]" : "text-slate-500"}`} />
                      <span className="font-semibold text-sm text-[#0A1622]">{resp.title}</span>
                    </div>
          ...
          {/* Comparison Table */}
          <div>
            <h3 className="text-base font-bold mb-5 uppercase tracking-wider text-slate-500">FDE vs Traditional Dev</h3>
            <div className="rounded-2xl border border-slate-200 overflow-hidden mb-5 shadow-sm">
              <div className="grid grid-cols-3 bg-[#F8F9FA] border-b border-slate-200">
                ...
                <div className="p-3.5 text-xs font-semibold text-slate-500 uppercase tracking-wider border-l border-slate-200">Traditional</div>
                <div className="p-3.5 text-xs font-semibold text-[#00B2A9] uppercase tracking-wider border-l border-slate-200">FDE</div>
              </div>
              {comparison.map((row, i) => (
                <div key={row.label} className={`grid grid-cols-3 ${i !== comparison.length - 1 ? "border-b border-slate-200" : ""}`}>
                  <div className="p-3.5 text-xs font-medium text-slate-600">{row.label}</div>
                  <div className="p-3.5 text-xs text-slate-500 border-l border-slate-200 flex items-start gap-1.5">
                    <span className="text-red-500 mt-0.5 shrink-0 text-[10px]">✕</span>
                    {row.traditional}
                  </div>
                  <div className="p-3.5 text-xs text-[#0A1622] font-medium border-l border-slate-200 flex items-start gap-1.5">
                    <span className="text-[#00B2A9] mt-0.5 shrink-0 text-[10px]">✓</span>
                    {row.fde}
                  </div>
                </div>
              ))}
            ...
            <div className="bg-[#F8F9FA] border border-slate-200 rounded-2xl p-5 shadow-sm">
              <p className="text-[#0A1622] font-semibold text-sm mb-1">We don't execute tickets.</p>
              <p className="text-slate-600 text-sm">We solve business problems with engineering excellence.</p>
            </div>
          </div>
        </div>
      </div>
